...
class Trainer:
    def __init__(self, model, train_loader, optimizer,
                 device, config:ProjectConfig, val_loader=None, logger=None):
    ...
    def train(self, epochs):
        ...
        if self.model_resume:
            ...
            if epochs <= start_epoch:
                self.logger.info("No epochs left to train. Increase the number of epochs in the configuration.")
                exit()
            self.logger.info(f"Resuming training from epoch {start_epoch}")

        for epoch in range(start_epoch, epochs):
            progress_bar = tqdm(self.train_loader, desc=f"Training Epoch {epoch + 1}", ncols=100)
            ...
            if self.val_loader:
                ...
                if val_loss < best_val_loss:
                    ...
                    self.logger.info(f"Best model saved to {self.config.project_directory}/model/best_model/best_model.pth")
            ...
            self.logger.info('Latent vector embeddings logged')

        # Log hyperparameters and final loss
        hparam_dict = self.get_hyperparameters()
    ...
    def get_hyperparameters(self):
        ...
        training_hparams = {field.name: getattr(self.config.training, field.name) for field in fields(self.config.training)}

        # Extract hyperparameters from model self.config
        model_hparams = {field.name: getattr(self.config.model, field.name) for field in fields(self.config.model)}
    ...
    def check_trained_model(self, model_path):
        ...
        if any(model_path.glob('*.pth')):
            ...
            self.logger.warning(f"Trained best model found: {latest_model}")
            self.logger.info(f"Loading model from {latest_model}")
            ...
            if resume_training.lower() == 'y':
                ...
                self.logger.info("Training aborted. Run evaluate.py to evaluate the model.")
                exit()
        else:
